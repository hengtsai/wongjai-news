#!/usr/bin/env python3
"""
新聞翻譯保守版 - 使用 Gemini 2.5 Flash，大幅降低請求頻率
"""
...
def call_gemini(system_prompt: str, user_prompt: str, max_tokens: int = 300) -> str:
    """呼叫 Gemini 2.5 Flash API，極保守的重試策略"""
    ...
    max_retries = 3  # 更少的重試次數
    base_delay = 5   # 更長的起始延遲
                    ...
                    if "content" in candidate and "parts" in candidate["content"]:
                        ...
                        if len(parts) > 0 and "text" in parts[0]:
                            ...
                            if text:
                                return text
                # 空回應也視為失敗，但不要立即重試
                print(f"      空回應，等待 {base_delay*2} 秒後繼續...")
...
def main():
    print("📰 新聞翻譯保守版啟動")
    print("=" * 70)
    print("模型: Gemini 2.5 Flash (保守模式)")
    print("語言: 繁體中文（台灣用語）、简体中文（中國大陸慣用語）、日本語、한국어")
    print("請求間隔: 文章間5秒，語言間3秒")
    ...
    # 批次處理設定
    BATCH_SIZE = 0  # 處理全部任務（今晚全部完成）
    if BATCH_SIZE > 0:
        print(f"批次處理: 今晚先處理前 {min(BATCH_SIZE, len(tasks))} 個任務")
        tasks = tasks[:BATCH_SIZE]
    
    # 開始翻譯
    print(f"\n開始翻譯（保守模式，預計時間: {len(tasks) * 10 / 60:.1f} 分鐘）...")
    print("=" * 70)
